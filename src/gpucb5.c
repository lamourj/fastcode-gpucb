...
void mmm_vect(int jj, int kk, int ll, int maxIter, float *sums, float *K, float *v) {
    ...
    for (int j = jj; j < jj + 8; j++) {
        ...
        const __m256 k_vector0 = _mm256_loadu_ps(K + kk * maxIter + ll); // load for next it
        ...
        const float sum_0 = sums[(kk % 8) * 8 + j_mod_8];
        const float sum_1 = sums[((kk + 1) % 8) * 8 + j_mod_8]; // load for next it
        const __m256 k_vector1 = _mm256_loadu_ps(K + (kk + 1) * maxIter + ll); // load for next it
        ...
        const float sum_2 = sums[((kk + 2) % 8) * 8 + j_mod_8]; // load for next it
        const __m256 k_vector2 = _mm256_loadu_ps(K + (kk + 2) * maxIter + ll); // load for next it
        ...
        const float sum_3 = sums[((kk + 3) % 8) * 8 + j_mod_8]; // load for next it
        const __m256 k_vector3 = _mm256_loadu_ps(K + (kk + 3) * maxIter + ll); // load for next it
        ...
        const float sum_4 = sums[((kk + 4) % 8) * 8 + j_mod_8];// load for next it
        const __m256 k_vector4 = _mm256_loadu_ps(K + (kk + 4) * maxIter + ll); // load for next it
        ...
        const float sum_5 = sums[((kk + 5) % 8) * 8 + j_mod_8];// load for next it
        const __m256 k_vector5 = _mm256_loadu_ps(K + (kk + 5) * maxIter + ll); // load for next it
        ...
        const float sum_6 = sums[((kk + 6) % 8) * 8 + j_mod_8]; // load for next it
        const __m256 k_vector6 = _mm256_loadu_ps(K + (kk + 6) * maxIter + ll); // load for next it
        ...
        const float sum_7 = sums[((kk + 7) % 8) * 8 + j_mod_8]; // load for next it
        const __m256 k_vector7 = _mm256_loadu_ps(K + (kk + 7) * maxIter + ll); // load for next it
